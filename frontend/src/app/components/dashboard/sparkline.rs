...
pub fn Sparkline(props: &SparklineProps) -> Html {
    ...
    let series_svg = props
        ...
        .iter()
        .map(|s| {
            let points = build_points(&s.values, lo, hi);
            if points.is_empty() {
                return Html::default();
            }
    ...
    let dots_overlay = props
        ...
        .filter_map(|s| {
            ...
            let (x, y) = point_at(&s.values, last, lo, hi)?;
    ...
    let hover_overlay = hover.and_then(|(idx, pointer_x)| {
        ...
        let top_y = props
            .series
            .iter()
            .filter_map(|s| point_at(&s.values, idx, lo, hi).map(|(_, y)| y))
            .fold(VIEW_H, f64::min);
        ...
        let markers = props
            ...
            .iter()
            .filter_map(|s| {
                let (x, y) = point_at(&s.values, idx, lo, hi)?;
